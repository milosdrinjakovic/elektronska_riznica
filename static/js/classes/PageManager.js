...
export default class PageManager {
  ...
  constructor(visual, book, p) {
    ...
    for(let i=0; i<5; ++i) {
      ...
      this.canvases.push({c, ctx: c.getContext('2d', {willReadFrequently: true, desynchronized: false})});
    }
    this.nextCanvas = 0;
  ...
  load(material, n) {
    ...
    Promise.resolve().then(()=> {
      if(o.texture) {
        ...
        o.wrapper.onChange = (image, canceled)=> {
          if(canceled) {
            if(o.state !== 'queuedForRender') {
              o.state = 'loaded';
              if(o.wrapper) {
                this.pushInRenderQueue(this.rendering);
              }
            }
  ...
  calcResolution(o) {
    let res;
    if(this.p.autoResolution.enabled) {
      const k = this.p.autoResolution.coefficient*this.p.zoom*Math.sqrt(this.visual.wnd.devicePixelRatio || 1)*Math.min((this.isSinglePage(o)? 1: 0.5)*this.visual.width()/o.widthTexels, this.visual.height()/o.heightTexels);
      res = {
        width: k*o.widthTexels,
        height: k*o.heightTexels
      };
      const minRes = this.p.autoResolution.min, maxRes = this.p.autoResolution.max;
      if(res.width<minRes) {
        res = {
          width: minRes,
